...
This design was reasonable but we found that it was difficult and slow to train the model. Instead, a simpler model design was used. The model took only one data plot as input at a time, and predicted the probability of the data plot being a null plot. In this case, the post-processing step was an indicator function for deciding whether the data plot is a null plot. By evaluating all $m$ plots of a lineup, multiple selections can be made.

### Model Architecture and Model Training

As mentioned in Section \ref{se:limitation}, modern computer vision model is built upon the deep neural network
