...
\item
  Propose a prototype of automatic visual inference system for evaluating lineups of residual plots of the classical normal linear regression model and study factors that affect the performance of the system with comparison to human subjects.
\item
  Extend and enhance the automatic visual inference system proposed in the first project to general lineup protocol problems by adopting image similarity assessed by computer vision models.
\item
...
\tightlist
\item
  The first project provides a prototype of automatic visual inference system for evaluating lineups of residual plots, with a focus on the classical normal linear regression model given it is one of the simplest predictive models. The core of the system
\end{enumerate}

Validation of the automatic system is made by comparing the power of human subject-assessed visual tests with the system-assessed visual tests.
...
\tightlist
\item
  The second project aims to develop an automatic visual inference system based on image comparison.
\end{enumerate}
...
The first project provides visual diagnostics for projection pursuit optimisation, with a focus on the guided tour. A guided tour is constructed by combining the optimisation, for choosing new planes to visit, with a geodesic interpolation between planes to provide a smooth movie of projections for visualisation. The visual diagnostics include static graphics and animations to understand different aspects of the optimisers: its progression on improving the index value, its coverage of the search space and the comparison of different optimisers in different scenarios. What is learned will be used to modify and improve the performance of the optimisers.

The second project aims to develop visual diagnostic tools for stochastic optimal control algorithms. In comparison to projection pursuit, optimisation in stochastic optimal control problems finds a sequence of discrete-valued decisions that are optimal at each time point. Exogenous variables that affect the decisions are also involved in the optimisation. The project will extend and build on the visual diagnostic tools developed in the first project to address stochastic optimal control optimisation.

The third project aims to use visual techniques to understand variable importance in stochastic optimal control problem. The project will study how to interpret the contribution of potentially dozens of variables to the decision at a particular time and how the future decision would change with a change in the exogenous variables.
