...
#### Let $K$ be the number of **independent evaluations** of a visual test, $s_i , i = 1, ..., K$ be the **number of selections of the evaluations**, and $X$ be the random variable denoting the number of detections. Then, the $p$-value of the visual test is given as: $$P(X\geq x) = \sum_{j=x}^{K}Pr(j|s_1,...,s_K).$$
...
#### We collect **400** lineup evaluations made by **20** participants in experiment I and **880** lineup evaluations made by **44** participants in experiment II. 

--

#### In total, **442** unique lineups are evaluated by **64** subjects.
...
#### All **decisions** made by $K\choose{K-j}$ different outcomes of $V_{K-j}$ are fitted by **binomial generalized linear models** with **natural logarithm of effect size** as the regressor.
...
.caption[
Fig. 10: _Quasibinomial GLM fitted for estimated power for different number of evaluations. The plot on the left is for the cubic model, the plot on the right is for the heteroskedasticity model. Power curves for different number of evaluations are drawn in different colors. The dashed line is the power of the conventional test._ 
]]
...
#### There is a huge gap between the conventional curves and the visual test curves when the natural logarithm of effect size is around $0$ for cubic model, and $2.5$ for heteroskedasticity model.
...
#### We propose a prototype **automatic visual inference system** for evaluating lineups of residual plots of classical normal linear regression. It currently only detects nonlinearity and heteroskedasticity.
...
#### Due to limited computing resources, we apply **transfer learning** to train five computer vision models. 

#### Five **pre-trained models** are used, including **VGG16**, **VGG19**, **Xception**, **ResNet152V2** and **InceptionV3**. 

#### Models are trained on **10000 null plots** and **10000 actual data plots** simulated from the cubic model and heteroskedasticity model. 20% training data is used as validation set.

#### **Image augmentation**:
- **random rotation angle** ranged from $0$ to $5$ degrees
- **random brightness** ranged from $95$% to $105$%

#### Model training is performed with **Tensorflow** and **Keras**. 
...
.caption[
Fig. 16: _Power of the automatic visual test compared to human visual test. The blue curve is the power of the automatic visual test. The dashed line represents the power of the conventional test. The plot on the left is for cubic model, and the plot on the right is for heteroskedasticity._ 
]]
...
.caption[
Fig. 17: _Power of the automatic visual test with different number of evaluations. The dashed line represents the power of the conventional test. The plot on the left is for cubic model, and the plot on the right is for heteroskedasticity._ 
]]
