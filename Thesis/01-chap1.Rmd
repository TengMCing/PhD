...
merged_survey %>%
  ...
  mutate(x_dist = factor(x_dist, levels = c("uniform", "normal", "lognormal", "neglognormal"))) %>%
  unnest_longer(col = p_value) %>%
  mutate(reject = eval_p_value(p_value, tol = 1e-2)) -> single_merged

glm(reject ~ a + b + c + e_sigma + x_dist + z_discrete + n + n_eval, data = filter(single_merged, type == "cubic"), family = binomial()) -> mod1_factor

```


```{r results="asis"}
glm(reject ~ a + b + x_dist + z_discrete + n + n_eval, data = filter(single_merged, type == "heteroskedasticity"), family = binomial()) -> mod2_factor

stargazer(mod1_factor, mod2_factor,header = FALSE,
          label = "tab:rejectfactor",
          title = "Logistic regressions with rejection as response variable and parameter settings as regressors fitted on the data collected from experiment I and II for both cubic and heteroskedasticity models.",
          style = "default",
          covariate.labels = c("a", "b", "c", 
                               "sigma", 
