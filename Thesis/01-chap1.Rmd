...
## P-value for Visual Test Allowed for Multiple Selections
...
```{r eval = FALSE}
small_survey_dat %>%
  mutate(p_hat = predict(mod1, newdata = small_survey_dat, type = "response")) %>%
  mutate(new_diff = ifelse(p_hat < 0.4, "hard", ifelse(p_hat > 0.8, "easy", "medium"))) %>%
  filter(difficulty != "extremely_easy") %>%
...
```{r eval = FALSE}
big_survey_dat %>%
  mutate(p_hat = predict(e1_high, newdata = big_survey_dat, type = "response")) %>%
  mutate(new_diff = ifelse(p_hat < 0.4, "hard", ifelse(p_hat > 0.8, "easy", "medium"))) %>%
  filter(difficulty != "extremely_easy") %>%
  mutate(me = new_diff == "easy" | new_diff == "medium") %>%
  count(set, me) %>%
