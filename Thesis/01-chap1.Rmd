...
[ET: Multiple observers, to me, is not about making smaller p-value possible with finite $m$ but if you only have a single observer, you are confounding the test with the visual ability of that single observer. In some ways, you can consider multiple observers as an ensemble approach and the decision is made on pooling the results from multiple "methods", each with their own abilities.]
...
where $g(.)$ is the logit link function $g(p) = log(p)  - log(1-p)$; $0 \leq p \leq 1$. $W_{li}$, $1 \leq i \leq K$, $1 \leq l \leq L$, is the covariate matrix including lineup-specific elements and demographic information of individuals, and $\delta$ is a vector of fixed effects. $Z_{li}$ is the random effects matrix, and $\tau ~ N(\boldsymbol{0}_{KL},\sigma_{\tau}\boldsymbol{I}_{KL\times KL})$.

[ET: need to double check this. Is $W_{li}$ a matrix or an element of the matrix? I wouldn't say "$Z_{li}$ is the random effects matrix". It's gotta be like random intercept model for lineup id and another random intercept for person id? ET to check.]
