...
e1_high <- glm(detect ~ I(log(effect_size)),
                data = filter(small_survey_dat, type == "higher order"),
                family = binomial)
e1_heter <- glm(detect ~ I(log(effect_size)),
...
According to the new GLM model given in Table \@ref(tab:e1glm) and Figure \@ref(fig:e1glmplot), it was found that the first experiment might be too difficult for participants as the detection rate was lower than what we estimated from the pilot study, especially for the heteroskedasticity model. Thus, for the second experiments, we relaxed the sample size requirements and increased the proportion of easy lineups. In addition, due to budget constraint, the maximum number of participants was $50$. With the new GLM model fitted with data collected from the first experiment, $40$ easy lineups, $25$ medium lineups and $15$ hard lineups were simulated for both cubic and heteroskedasticity model in the second experiment. $49$ participants were recruited but $6$ were withdrew.
...
for (i in 1:length(lineup_dat_big)) {
  ...
  ori_eff <- c(ori_eff, lineup_dat_big[[i]]$metadata$effect_size)
  current_eff <- c(current_eff, tmp_eff)
  prm_c <- c(prm_c, lineup_dat_big[[i]]$metadata$c)
