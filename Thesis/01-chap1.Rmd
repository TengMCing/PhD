...
```{r cubic1, fig.cap = "cubic 1", fig.height=6.3}
library(visage)
library(tidyverse)
...
```{r cubic2, fig.cap = "cubic 2", fig.height=7.875}
expand.grid(c=seq(0.5,1.5,0.25),
            a=seq(-1,1,1),
            b=seq(-1,1,1),
            e_sigma=c(0.5)) %>%
...
```{r cubic3, fig.cap = "cubic 3", fig.height=7.875}
expand.grid(c=c(1),
            a=seq(-1,1,1),
            b=seq(-1,1,1),
            e_sigma=c(0.25,0.5,1,2)) %>%
  arrange(a,b,c,desc(e_sigma)) %>%
  rowwise() %>%
  mutate(plots = list(make_plot(a,b,c,e_sigma))) -> result

result$plots %>% patchwork::wrap_plots(ncol=4)
```
...
mod$plot(dat, theme = theme_light(), size = 1) +
