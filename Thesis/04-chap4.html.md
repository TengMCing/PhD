...
Most statistical software offers graphical tools for diagnosing regression models. For example, the base R package `stats` (citation here) automatically generates four diagnostic plots for linear models (lm objects): a residuals vs. fitted values plot, a Scale-Location plot (square root of absolute residuals vs. fitted values), a normal Q-Q plot (citation here), and a residuals vs. leverage plot. Additionally, packages hosted on The Comprehensive R Archive Network (CRAN) (citation here) such as `jtools` provide a more comprehensive and aesthetically enhanced set of diagnostic plots. However, very few of them offer automated assessment for these plots. Users often need to manually inspect and interpret the plots, which can lead to inconsistent conclusions due to varying levels of domain and statistical knowledge. This variability makes it challenging to achieve standardized results from plot diagnostics.
...
The primary goal of `autovi` is to provide rejection decisions and $p$-values for testing the null hypothesis that a regression model is correctly specified. While the classical normal linear regression model is popular and widely used, it represents a limited class of models. Therefore, the R package `autovi` is designed to be highly modular and customizable, allowing it to handle a variety of residual diagnostic tasks.

### Modularized Data Pipeline

In this section, we will discuss the data pipeline of `autovi`. It begins with the user providing a regression model and concludes with the delivery of assessment results to the user. An overview diagram of the data pipeline can be found in Figure . 

To allow the package to adapt to various scenarios, the data pipeline is designed to be modular, with each module making minimal assumptions about the inputs from previous modules. This modularity ensures that any customizations by the user can be made without affecting the rest of the data pipeline.

The data pipeline consists of eight core modules: 'Initialize', 'Get Fitted Values and Residuals', 'Get Data', 'Run Null Method', 'Plot', 'Save Plots', 'Compute Auxiliary', and 'Check'. The first and last modules are predefined methods that can not be overridden. However, users can interact with them directly and influence their behaviour through arguments passed to these functions. The remaining six modules are designed to be customizable, allowing users to override them to meet their specific needs.

#### Initialize

In the initialization step, the user needs to construct a checker by supplying a regression model object - typically an `lm` object representing the result of a linear regression model and a trained computer vision model compatible with the `Keras` API.
...
#### Get Data, Fitted values, and Residuals 

We did not make any assumptions about the type of regression model object provided to the checker since  
