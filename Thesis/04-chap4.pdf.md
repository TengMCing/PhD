...
::: {.cell-output-display}
![](04-chap4_files/figure-pdf/unnamed-chunk-13-1.pdf){fig-pos='H'}
:::
:::
...
```
[1] "/var/folders/61/bv7_1qzs20x6fjb2rsv7513r0000gn/T//RtmpLasgWU/file151e511e4dd12.png"
```
...
::: {.cell-output-display}
![](04-chap4_files/figure-pdf/unnamed-chunk-24-1.pdf){fig-pos='H'}
:::
:::
...
::: {.cell-output-display}
![](04-chap4_files/figure-pdf/unnamed-chunk-30-1.pdf){fig-pos='H'}
:::
:::
...
In the result panels shown in [@fig-autovi-web-workflow-example]D, the first row of the table is the most crucial to check, as it provides the visual signal strength and the rank of the true residual plot among the other plots. The summary text beneath the table provides the $p$-value, which can be used for quick decision-making. The lineup is for manual inspection, and the user should see if the true residual plot is visually distinguishable from the other plots, to confirm if the model violation is serious. The density plot in [@fig-autovi-web-workflow-example]E offers a more robust result, allowing the user to compare the distribution of bootstrapped visual signal strength with the distribution of null visual signal strength. Finally, the grayscale attention map can be used to check if the target visual features, like the non-linearity present in the lineup example, are captured by the computer vision model, ensuring the quality of the assessment.
