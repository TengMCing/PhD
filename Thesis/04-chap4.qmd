...
if (!file.exists("cached_data/autovi_check_demo_8.rds")) {
  ...
  checker$check(null_draws = 100L,
                boot_draws = 100L,
                extract_feature_from_layer = "global_max_pooling2d")
  saveRDS(checker$check_result, "cached_data/autovi_check_demo_8.rds")
} else {
  checker$check_result <- readRDS("cached_data/autovi_check_demo_8.rds")
}
checker$feature_pca()
```
...
```{r fig-autovi-web-gradient-hide, out.width = "100%", fig.cap = "The attention map is hidden if the assessment indicates a $p$-value greater than 0.05. A button is available to toggle the display of the attention map."}
...
The workflow of `autovi.web` is designed to be straightforward, with numbered steps displayed in each panel. Since the design details have already been covered in @sec-autovi-web-design, we will not repeat the functionality of each panel here.

Two example datasets are provided by the web application, as mentioned in @sec-autovi-web-design. The single residual plot example uses the `dino` dataset from the R package `datasauRus` [@datasaurus]. The lineup example uses residuals from a simulated regression model that has a non-linearity issue. We will walk through the lineup example to further demonstrate the workflow of the web application.
