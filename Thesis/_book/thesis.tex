...
The multiple individuals approach avoids the limit of \(m\), while provides visual tests with \(\alpha\) much smaller than \(0.05\). In fact, \(\alpha\) decreases exponentially as \(K\) increases. With just \(4\) individuals and \(20\) data plots in a lineup, the \(p\)-value could be as small as \(0.0001\).
...
\item
  ...
  \item
    Use simulated data plot to train model with null or not null as target -\textgreater{}
  \end{itemize}
\end{itemize}
