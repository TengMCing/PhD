...
```{r batch, fig.cap = "Boxplot of weighted propotion of detect for lineups over different data collection periods."}
vi_survey %>%
...
```{r sensitive, fig.cap = "Change of $p$-values with $\\hat{\\alpha}$ inflated by 25% and deflated by 25%."}
vi_survey %>%
  filter(!attention_check) %>%
  filter(!null_lineup) %>%
  filter(x_dist == "uniform") %>%
