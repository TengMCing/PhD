...
vi_survey %>%
  ...
  kableExtra::kable(format = "latex", booktabs = TRUE, label = "pronoun", caption = "Summary of pronoun distribution of subjects recuritted in this study.")
```
...
vi_survey %>%
  ...
  labs(fill = "Quantiles") +
  scale_fill_discrete(labels = c("50%", "25% - 75%", "12.5% - 87.5%", "6.25% - 93.75%")) +
...
```{r sensitive, fig.cap = "\\label{fig:sensitive}Change of $p$-values with $\\hat{\\alpha}$ multiplied by $0.125$, $0.25$, $0.5$, $1.125$, $1.25$ and $1.5$."}
upper_lower_p %>%
  left_join(vi_survey %>%
  filter(!attention_check) %>%
  filter(!null_lineup) %>%
  ...
  ggplot() +
  geom_point(aes(p_value, adjusted_p_value), alpha = 0.2) +
  geom_abline() +
  facet_wrap(~modified) +
  scale_x_sqrt() +
  scale_y_sqrt() +
...
upper_lower_p %>%
  ...
  mutate(case = ifelse(!adjusted_reject, "Propotion of lineups with $p$-value < 0.05 but adjusted $p$-value > 0.05", "Propotion of lineups with $p$-value > 0.05 but adjusted $p$-value < 0.05")) %>%
...
poly_x_dist <- poly_conv_sim %>%
  ...
  filter(name == "RESET test") %>%
  mutate(reject = value <= 0.05) %>%
  select(effect_size, name, x_dist, reject) %>%
...
```{r conv-x-dist, fig.cap = "Change of power of conventional tests under different fitted value distribution for non-linearity model and heteroskedasticity model.", fig.height = 3}
bind_rows(poly_x_dist, heter_x_dist) %>%
  mutate(type = factor(type, levels = c("non-linearity", "heteroskedasticity"))) %>%
  ggplot() +
