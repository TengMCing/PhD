...
Surprisingly, the visual test rejects `r p_value_cmp_dat %>% filter(type == "heteroskedasticity") %>% filter(conv_reject == "Not") %>% count(reject) %>% summarise(pct=round(n[1]/(n[1]+n[2])*100, 0))`% (i.e. 1 out of 32) of lineups where the conventional test does not reject. This lineup shows relatively strong heteroskedasticity pattern as displayed in Figure \ref{fig:heter-example}. The visual test rejects it with a $p\text{-value} = 0.026$, but the Breusch-Pagan test produces a $p$-value slightly above 5%. 
