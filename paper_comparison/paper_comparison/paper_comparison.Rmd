...
for (i in target_sigma) {
  ...
  VI_MODEL$plot(poly_model(shape = 2, x = rand_uniform())$
                  gen(n = 100, computed = select(mutate(ex_dat, e = sqrt(i) * e), x, e)), theme = theme_light(base_size =  5), 
              remove_axis = TRUE, 
              remove_grid_line = TRUE) +
...
```{r polypower, fig.height = 3, fig.width = 12, fig.cap="..."}
patchwork::wrap_plots(append(plist, list(p), after = 0), 
                      design = map(1:11, ~patchwork::area(16, .x, 22, .x)) %>%
                        reduce(c) %>%
                        c(patchwork::area(3, 1, 15, 11), .))
```
...
```{r heterpower, fig.height = 3, fig.width = 12, fig.cap="..."}
patchwork::wrap_plots(append(plist, list(p), after = 0), 
                      design = map(1:11, ~patchwork::area(16, .x, 22, .x)) %>%
                        reduce(c) %>%
                        c(patchwork::area(3, 1, 15, 11), .))
