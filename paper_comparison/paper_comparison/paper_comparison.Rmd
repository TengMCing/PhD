...
Shape of the non-linearity pattern is another factor contributed to the simulation model. Figure \ref{fig:poly-power-j} 

```{r poly-power-j, fig.height = 4, fig.cap = "Power of conventional tests and visual tests on lineups produced by the non-linearity model with four different shapes controlled by the parameter $j$."}
...
poly_conv_sim %>%
  ...
  mutate(shape = c("U", "S", "M", "Triple-U")[shape]) %>%
  mutate(conv_or_not = "Visual")) %>%
  mutate(shape = factor(shape, levels = c("S", "U", "M", "Triple-U"))) %>%
