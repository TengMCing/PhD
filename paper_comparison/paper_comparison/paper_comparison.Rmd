...
survey_single_pvalue <- survey %>%
  calc_p_value_multi(lineup_id = "exp_lineup_id", 
                     ...
                     n_sim = 100000,
                     cache_env = env) %>%
  left_join(lineup_info, by = c("lineup_id" = "exp_lineup_id")) %>%
  mutate(reject = eval_p_value(p_value, tol = 1e-02))
...
# set.seed(20002)
# mod <- cubic_model(a = -3, b = 0, c = 1, sigma = 0.5, 
#                    x = rand_uniform(-1, 1), z = rand_uniform(-1, 1))

# fixed_sample <- mod$gen(100)
plot_list <- list()
...
for (a in seq(-3, 3, 1)) {
  for (b in seq(-3, 3, 1)) {
    ...
    mod <- cubic_model(a = a, b = b, c = 0, sigma = 0.5, 
                   x = rand_uniform(-1, 1), z = rand_uniform(-1, 1))

    plot_list[[paste(a, b, sep = "_")]] <- mod$gen(1000, fit_model = TRUE) %>%
    mod$plot("resid", remove_grid_line = TRUE, theme = theme_light(), remove_axis = TRUE) +
    ggtitle(glue::glue("a: {mod$prm$y$sym$a}, b: {mod$prm$y$sym$b}")) +
    ylim(c(-20, 20))
  }
}
...
for (c in seq(0, 2, 0.5)) {
    mod$prm$y$sym$c <- c
    plot_list[[paste(c)]] <- mod$gen(100, fit_model = TRUE, 
                                     computed = list(e = fixed_sample$e,
                                                     x = fixed_sample$x,
                                                     z = fixed_sample$z)) %>%
    mod$plot("resid", remove_grid_line = TRUE, theme = theme_light(), remove_axis = FALSE) +
    ggtitle(glue::glue("c: {mod$prm$y$sym$c}")) 
}
...
for (b in c(0, 0.5, 1, 1.5, 2, 2.5, 3, 10)) {
    
    set.seed(20002)
    mod <- heter_model(a = 1, b = b, x = rand_uniform(-1, 1))
    
    plot_list[[paste(b)]] <- mod$gen(1000, fit_model = TRUE) %>%
    mod$plot("resid", remove_grid_line = TRUE, theme = theme_light(), remove_axis = TRUE) +
    ggtitle(glue::glue("b: {b}"))
}
...
```{r}
survey_single_pvalue %>%
  mutate(reject = as.numeric(reject)) %>%
  filter(n_eval >= 1) %>%
  ggplot() +
  geom_histogram(aes(log_effect_size, ..count../10), binwidth = 0.1) +
  geom_point(aes(log_effect_size, reject), alpha = 0.2) +
  ...
  geom_smooth(aes(log_effect_size, reject), method = "glm", se = FALSE, method.args = list(family = binomial())) +
...
```{r}
sim_conv <- sim_conv %>%
  mutate(dis_b = cut(abs(b), breaks = c(0, 1, 2, 3)))

kernel_layer <- function(dis_b_index) {
  geom_line(data = kernel_regression_cubic(filter(sim_conv, as.numeric(dis_b) == dis_b_index), 
                                           0.4, 
                                           x = quote(abs(a))), 
            aes(x, reject, col = levels(sim_conv$dis_b)[dis_b_index]), 
            linetype = 2)
}
...
survey_single_pvalue %>%
  ...
  ggplot() +
  # geom_histogram(aes(log_effect_size, ..count../10), binwidth = 0.1) +
  geom_point(aes(abs(a), reject), alpha = 0.2) +
  kernel_layer(1) +
  kernel_layer(2) +
  kernel_layer(3) +
  geom_smooth(aes(abs(a), reject, col = dis_b), method = "glm", se = FALSE, method.args = list(family = binomial())) +
  facet_grid(n~e_sigma, scales = "free_x") +
  labs(col = "abs(b)")
```
