...
for (i in target_sigma) {
  ...
  VI_MODEL$plot(poly_model(shape = 2, x = rand_uniform())$
                  gen(n = 100, computed = select(mutate(ex_dat, e = sqrt(i) * e), x, e)), theme = theme_light(), 
              remove_axis = TRUE, 
              remove_grid_line = TRUE) +
...
```{r, fig.cap="..."}
vi_survey %>%
  mutate(x_dist = ifelse(x_dist == "even_discrete", "discrete uniform", x_dist)) %>%
  group_by(unique_lineup_id) %>%
  summarise(across(everything(), first)) %>%
  filter(!null_lineup) %>%
  filter(!attention_check) %>%
  ...
  geom_smooth(data = mutate(poly_conv_sim, log_effect_size = log(effect_size)) %>%
                pivot_longer(F_p_value:SW_p_value) %>%
                mutate(name = gsub("_p_value", " test", name)) %>%
                mutate(type = "polynomial") %>%
                mutate(x_dist = ifelse(x_dist == "even_discrete", "discrete uniform", x_dist)) %>%
                bind_rows(mutate(heter_conv_sim, log_effect_size = log(effect_size)) %>%
                pivot_longer(F_p_value:SW_p_value) %>%
                mutate(name = gsub("_p_value", " test", name)) %>%
