...
polynomials %>%
  group_by(lineup_id) %>%
  summarise(dd = first(abs(effect_size - exp(2)))) %>%
  arrange(dd)
```
...
conv_test_plot() %>%
  ...
  geom_vline(xintercept = 6) -> base_plot
...
VI_MODEL$plot_lineup(polynomials_lineup$lineup_415$data,
                     ...
                     theme = theme_light(base_size = 5),
                     remove_axis = TRUE) +
  ggtitle(bquote(log[e](E) == 2~","~"F-test p-value" == .(format(filter(polynomials, lineup_id == 415)$conventional_p_value[1], digits = 3))~", Visual p-value " == .(format(filter(polynomials, lineup_id == 415)$p_value[1], digits = 3)))) -> bottom_plot_2

VI_MODEL$plot_lineup(polynomials_lineup$lineup_494$data,
                     size = 0.1,
                     stroke = 0.2,
                     remove_grid_line = TRUE,
                     theme = theme_light(base_size = 5),
                     remove_axis = TRUE) +
  ggtitle(bquote(log[e](E) == 4~","~"F-test p-value" == .(format(filter(polynomials, lineup_id == 494)$conventional_p_value[1], digits = 3))~", Visual p-value " == .(format(filter(polynomials, lineup_id == 494)$p_value[1], digits = 3)))) -> bottom_plot_3

VI_MODEL$plot_lineup(polynomials_lineup$lineup_101$data,
                     size = 0.1,
                     stroke = 0.2,
                     remove_grid_line = TRUE,
                     theme = theme_light(base_size = 5),
                     remove_axis = TRUE) +
  ggtitle(bquote(log[e](E) == 6~","~"F-test p-value" == .(format(filter(polynomials, lineup_id == 101)$conventional_p_value[1], digits = 3))~", Visual p-value " == .(format(filter(polynomials, lineup_id == 101)$p_value[1], digits = 3)))) -> bottom_plot_4
