...
A paragraph on what hypothesis(es) are being tested.

Generally, by looking at the residuals we are concerned with 

$H_o:$ The model specification is correct

against a broad alternative that something is missing:

$H_a:$ The model specification is not correct

The conventional tests are only equipped to measure specific departures, and thus have a more specific hypotheses being tested:
...
map(1:7, ~patchwork::area(21, .x, 22, .x)) %>%
  reduce(c) %>%
  c(patchwork::area(3, 1, 20, 7), .) %>%
...
target_b <- c(0.27, 0.74, 2.01, 5.46, 14.84, 40.34, 109.66)
