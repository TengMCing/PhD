...
Depending on the natural logarithm of the effect, lineups are classified into three categories: easy (< 1.5), moderate (1.5 - 3.5) and difficult (> 3.5). Figure \ref{fig:p-value-comparison} provides a mosaic plot showing the rejection rate of visual tests and conventional tests at different difficulty levels.

### Easy lineup

For easy lineups produced by the non-linearity model, 65% are rejected by both the conventional and visual tests, while 9% are accepted by both tests. Thus, 74% of the time two tests will agree with each other. There are 16% of lineups rejected only by the conventional test. 

Interestingly, in spite of the greater power of the conventional test, 10% of the lineups are rejected only by the visual test. This means among those accepted by the conventional test, 51% are rejected by the visual test. Recall that the RESET test used in our analysis includes powers of fitted values up to four. However, the "M" and "Triple-U" shapes constructed from the Hermite polynomials contain powers of regressor up to six and 18 respectively. In fact, by including the powers of fitted values up to six, all cases will be rejected by the RESET test. And from the example plot displayed in Figure \ref{fig:easy-poly-example}, we can observe panel three is distinctly different from others exhibiting a "M" shape. It clearly suggests users of the RESET test and any other residual-based conventional tests that require the specification of variables of interest to check the residual plot before conducting the test, such that the correct choice of variables can be made. 

```{r easy-poly-example, fig.height = 5, fig.cap = "An example easy lineup produced by the non-linearity model not rejected by the conventional test but rejected by the visual test. Panel three exhibits strong non-linear pattern."}
...
```{r easy-heter-example1, fig.height = 5, fig.cap = "An example easy lineup produced by the heteroskedasticity not rejected by the conventional test but rejected by the visual test. Panel 17 exhibits strong heteroskedasticity pattern."}
...
```{r easy-heter-example2, fig.height = 5, fig.cap = "An example easy lineup produced by the heteroskedasticity not rejected by the conventional test but rejected by the visual test. Panel 18 contains a distinct outlier."}
...
For easy lineups produced by the heteroskedasticity model, 62% are rejected by both the conventional test and the visual test, and 7% are accepted by both, result in 69% agreement rate. Lineups rejected by only conventional test are 30%. 1% of lineups are accepted by the conventional test but rejected by the visual test. 

Among those 1% lineups, which are two lineups, one shows detectable heteroskedasticity pattern as displayed in Figure \ref{fig:easy-heter-example1}. Another contain data plot with an noticeable outlier, but the "Right-triangle" shape is not obvious as shown in Figure \ref{fig:easy-heter-example2}. The $p$-values of Breusch-Pagan test for these two lineups are $0.055$ and $0.135$. Compared to the visual test, its $p$-values are $0.026$ and $0.012$. The Breusch-Pagan test should reject the one with obvious heteroskedasticity signal, but it yields a $p$-value slightly above 5%.  

### Moderate lineup

With the increase of the difficulty level, the sensitivity of conventional tests enlarge the difference in rejection rates. For moderate lineup produced by the non-linearity model, only 8% and 28% are rejected and accepted by both respectively. We can observe a decline in agreement rate to 36%. A huge amount of lineups are accepted by the visual test. 63% of lineups rejected only by the conventional test. There are 1% of lineups which rejected only by conventional tests suffer the same RESET test issue as mentioned earlier.  

For those produced by the heteroskedasticy model, the corresponding numbers are 23%, 31%, 54%, 45% and 1%. Only one lineup rejected by the visual test but accepted by the conventional test. It shows zero heteroskedasticity patterns, but contains a few outliers at the bottom of the data plot as illustrated in panel 14 of Figure \ref{fig:moderate-heter-example1}. Two out of five subjects correctly identify this lineup yielding a $p$-value equal to $0.044$.

```{r moderate-heter-example1, fig.height = 5, fig.cap = "An example easy lineup produced by the heteroskedasticity not rejected by the conventional test but rejected by the visual test. Panel 14 contains a few outliers at the bottom of the panel."}
...
Visual test is insensitive to lineups with small effect. When it comes to difficult lineups produced by the non-linearity model, none of them are rejected by the visual test. There are 73% of lineups accepted by both tests, and 26% rejected by the conventional test. The agreement rate rise up to 73%. 

For lineups produced by the heteroskedasticity model, 3% are rejected by both, 71% are accepted by both, 24% are rejected only by the conventional test and 2% are rejected only by the visual test, which are two lineups. 

As shown in \ref{fig:difficult-heter-example1} and \ref{fig:difficult-heter-example2}, residual plots of both lineups are constructed with $X_{raw} \sim lognormal(0, 0.6^2)/3$. Though the designed patterns are "Left-triangle" and "Butterfly", both of them are invisible due to the small effect. The residual plots are dominated by the skewed distribution. Further, no distinguishable visual patterns, such as outliers, are detectable from the data plot of these two lineups, which are panel 5 and panel 15. The data plot in these two lineups are not the most different plot from our point of view either. However, three out of five and two out of five subjects correctly identify the data plot for these two lineups respectively. But the reasons for selecting the plot provided by the subjects are not consistent. Therefore, with the current data, we can not explain why subjects can correctly identify the data plot of these two lineups. Collecting more data in future study may help to answer this question.

```{r difficult-heter-example1, fig.height = 5, fig.cap = "An example easy lineup produced by the heteroskedasticity not rejected by the conventional test but rejected by the visual test. Panel 14 contains a few outliers at the bottom of the panel."}
...
```{r difficult-heter-example2, fig.height = 5, fig.cap = "An example easy lineup produced by the heteroskedasticity not rejected by the conventional test but rejected by the visual test. Panel 14 contains a few outliers at the bottom of the panel."}
