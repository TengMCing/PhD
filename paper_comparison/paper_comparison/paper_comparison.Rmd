...
# Generate and plot data for shape 2. Reuse x and e.
pp2 <- poly_model(shape = 2, 
                  x = {
                    raw_x <- rand_uniform(-1, 1); 
                    closed_form(~stand_dist(raw_x))
                    }, 
...
# Generate and plot data for shape 3. Reuse x and e.
pp3 <- poly_model(shape = 3, 
                  x = {
                    raw_x <- rand_uniform(-1, 1); 
                    closed_form(~stand_dist(raw_x))
                    }, 
...
# Generate and plot data for shape 4. Reuse x and e.
pp4 <- poly_model(shape = 4, 
                  x = {
                    raw_x <- rand_uniform(-1, 1); 
                    closed_form(~stand_dist(raw_x))
                    }, 
...
# Plot the data for uniform distribution
...
# Generate and plot data for normal distribution
dp2 <- poly_model(shape = 1, 
                  x = {
                    raw_x <- rand_normal(sigma = 0.3); 
                    closed_form(~stand_dist(raw_x))
                    }, 
                  sigma = 0.5)$
  gen(300, computed = select(ori_dat, e)) %>%
  VI_MODEL$plot() +
  ggtitle("B. Normal") +
  theme_light() +
  xlab("Fitted values") +
...
# Generate and plot data for lognormal distribution
dp3 <- poly_model(shape = 1, 
                  x = {
                    raw_x <- rand_lognormal(sigma = 0.6);
                    closed_form(~stand_dist(raw_x/3 - 1))
                    }, 
...
# Plot data for 300 observations
np300 <- VI_MODEL$plot(ori_dat) + 
  ggtitle(quote(n~"="~300)) + 
  theme_light() +
  xlab("Fitted values") +
  ylab("Residuals") +
...
# Generate and plot data for 50 observations
np50 <- poly_model(shape = 2, 
                   x = {
                     raw_x <- rand_uniform(-1, 1);
                     closed_form(~stand_dist(raw_x))
                     }, 
                   sigma = 1.5)$
  gen(50, computed = select(ori_dat[1:50, ], x, e)) %>%
  VI_MODEL$plot() +
  ggtitle(quote(n~"="~50)) + 
  theme_light() +
  xlab("Fitted values") +
...
# Generate and plot data for 100 observations
np100 <- poly_model(shape = 2, 
                    x = {
                      raw_x <- rand_uniform(-1, 1);
                      closed_form(~stand_dist(raw_x))
                      }, 
                    sigma = 1.5)$
  gen(50, computed = select(ori_dat[1:100,], x, e)) %>%
  VI_MODEL$plot() +
  ggtitle(quote(n~"="~100)) + 
  theme_light() +
  xlab("Fitted values") +
...
patchwork::wrap_plots(np50, np100, np300, ncol = 3)
```
...
# Generate and plot data for sigma 1
sp2 <- poly_model(shape = 1, 
                  x = {
                    raw_x <- rand_uniform(-1, 1); 
                    closed_form(~stand_dist(raw_x))
                    }, 
                  sigma = 1)$
  gen(300, computed = select(ori_dat, x)) %>%
  VI_MODEL$plot() +
...
# Generate and plot data for sigma 2
sp3 <- poly_model(shape = 1, 
                  x = {
                    raw_x <- rand_uniform(-1, 1); 
                    closed_form(~stand_dist(raw_x))
                    }, 
...
# Generate and plot data for sigma 4
sp4 <- poly_model(shape = 1, 
                  x = {
                    raw_x <- rand_uniform(-1, 1); 
                    closed_form(~stand_dist(raw_x))
                    }, 
...
# Generate and plot data for b = 1
bp2 <- heter_model(a = 0, 
                   x = {
                     raw_x <- rand_uniform(-1, 1); 
                     closed_form(~stand_dist(raw_x))
...
# Generate and plot data for b = 4
bp3 <- heter_model(a = 0, 
                   x = {
                     raw_x <- rand_uniform(-1, 1); 
                     closed_form(~stand_dist(raw_x))
...
# Generate data and plot data for b = 16
bp4 <- heter_model(a = 0, 
                   x = {
                     raw_x <- rand_uniform(-1, 1); 
                     closed_form(~stand_dist(raw_x))
...
# Generate data and plot data for b = 64
bp5 <- heter_model(a = 0, 
                   x = {
                     raw_x <- rand_uniform(-1, 1); 
                     closed_form(~stand_dist(raw_x))
...
Therefore, we plan to collect 6 more evaluations on the 279 lineups with $X_{raw} \sim U(-1,1)$, result in $(36 \times 20 + (4 \times 4 \times 3 + 3 \times 5 \times 3) \times 3 \times 6) / (20-2) = `r (36 * 20 + (4 * 4 * 3 + 3 * 5 * 3) * 3 * 6) / 18`$ subjects recruited for experiment III.
