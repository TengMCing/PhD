...
Example residual plots given in Figure \ref{fig:commondepartures} are examined by the corresponding F-test, Breusch-Pagan test and Shapiro--Wilk test as shown in Table \ref{tab:example-residual-plot-table}. In the example, both the Breusch-Pagan test and the Shapiro--Wilk test rejects the null hypothesis for departures they do not intend to examine. As discussed in @cook1982residuals, most residual-based tests for a particular type of departure from model assumptions are sensitive to other types of departures. It is likely the null hypothesis is correctly rejected but for the wrong reason, which is known as the "Type III error". Additionally, outliers will often incorrectly trigger the rejection of the null hypothesis despite the residuals are well-behaved [@cook_applied_1999]. This can be largely avoided in diagnostic plots as experienced analysts can evaluate the acceptability of assumptions flexibly, even in the presence of outliers.


<!-- report 0 when the value is small -->

<!-- F-test -> model structrual test -->
...
data.frame(plot = c("B", "C", "D"), 
  ...
  mutate(f = format(f, digits = 2), b = format(b, digits = 2), s = format(s, digits = 2)) -> table_dat


  table_dat$f <- kableExtra::cell_spec(table_dat$f, color = ifelse(as.numeric(table_dat$f) < 0.05, "red", "black"))
  table_dat$b <- kableExtra::cell_spec(table_dat$b, color = ifelse(as.numeric(table_dat$b) < 0.05, "red", "black"))
  table_dat$s <- kableExtra::cell_spec(table_dat$s, color = ifelse(as.numeric(table_dat$s) < 0.05, "red", "black"))

  table_dat %>%
  knitr::kable(col.names = c("Residual plots", "Departures", "F-test ", "Breusch-Pagan test", "Shapiro–Wilk test"),
               escape = FALSE,
               format = "latex",
               booktabs = TRUE, 
               caption  = 'Example residual plots examined by the corresponding conventional tests. P-values smaller than 0.05 are colored in red.', 
