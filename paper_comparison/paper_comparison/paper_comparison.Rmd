...
```{r different-dist, fig.height = 2, fig.cap="Variations in fitted values, that might affect perception of residual plots. Four different distribution are used."}
...
Figure \ref{fig:example-poly-lineup} demonstrates one of the lineups used in experiment I. This lineup is produced by the non-linearity model under $j = 6$ and fitted values follow a uniform distribution. The data plot location is $2^3 - 4$. All five subjects correctly identify the data plot from this lineup.
...
Therefore, we plan to collect 6 more evaluations on the 279 lineups with fitted values follow a uniform distribution, result in $(36 \times 20 + (4 \times 4 \times 3 + 3 \times 5 \times 3) \times 3 \times 6) / (20-2) = `r (36 * 20 + (4 * 4 * 3 + 3 * 5 * 3) * 3 * 6) / 18`$ subjects recruited for experiment III.
...
In the following analysis, lineups with fitted values follows a uniform distribution will be the focus of next two sections. Visual patterns are more likely to be revealed under a uniform distribution. Additionally, we have collected extra evaluations on these lineups, which will result in more reliable analysis. Analysis of lineups with other fitted value distributions can be found in Section \ref{fitted-value-distribution}. 
...
Figure \ref{fig:polypower} shows the estimated power of visual test on lineups produced by the non-linearity model with fitted values follow a uniform distribution, against the natural logarithm of the effect $log_e(E)$, with a 5% significance level. At the bottom of the figure \ref{fig:polypower}, there are a sequence of example residual plots with increasing levels of $log_e(E)$. Readers can evaluate them from left to right and determine at which level the departure from a good residual plot becomes detectable.
...
For the non-linearity model, the power curve of RESET test climbs aggressively from
...
For lineups produced by the non-linearity model, conventional tests reject `r p_value_cmp_dat %>% filter(type == "heteroskedasticity") %>% count(conv_reject) %>% summarise(pct=round(n[1]/(n[1]+n[2])*100, 0))`% 
...
## Effect of other factors used in the simulated model
...
For the non-linearity model, we do not observe significant difference in power of visual test between distributions, except for the discrete uniform distribution. It is possibility due to the fact that humans have difficulties in recognising visual patterns from discreteness, which makes the shape disconnected and incomplete (ref?). In terms of conventional tests, the discrete uniform distribution is instead the one with the greatest power, followed by the lognormal distribution, the uniform distribution and the normal distribution.
...
In regards of the type of the simulated model, the power of visual tests under different distributions of regressor are always lower than the corresponding conventional tests, which is consistent with the result from the previous sections.

```{r different-x-dist-poly-power, fig.height = 4, fig.cap = "Power of conventional tests and visual tests on lineups produced by the non-linearity model under different fitted value distributions."}
