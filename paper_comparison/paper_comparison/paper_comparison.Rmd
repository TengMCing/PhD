...
VI_MODEL$plot(ori_dat) + ggtitle(quote(He[2]:"U shape")) -> p1
...
VI_MODEL$plot(poly_model(shape = 4, x = {raw_x <- rand_uniform(-1, 1); closed_form(~stand_dist(raw_x))}, sigma = 0.05)$gen(300, computed = select(ori_dat, x, e))) +
  ggtitle(quote(He[18]:"Triple-U shape")) -> p4
...
VI_MODEL$plot(ori_dat) + ggtitle(quote(sigma~"="~0.5)) -> p1
...
VI_MODEL$plot(ori_dat) + ggtitle(quote(U(-1,1))) -> p1
...
[table ref here] tabulates the number of subjects, genders, and lineups evaluated after applying the data screening procedure.
