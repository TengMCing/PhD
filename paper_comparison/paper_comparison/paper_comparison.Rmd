...
patchwork::wrap_plots(mod$plot(dat, theme = theme_light()) + ggtitle("Good residuals"),
                      mod$plot(mod$
                                 ...
                                 gen(300, computed = select(dat, x, e)), theme = theme_light()) + 
                        ggtitle(paste("F-test p-value =", 
                                      mod$
                                        test(mod$
                                               gen(300, computed = select(dat, x, e)))$p_value %>% 
                                        format(digits = 2))),
                      mod$
                        plot(heter_model(b = 64)$
                               gen(300, computed = select(dat, x, e)), theme = theme_light()) + 
                        ggtitle(paste("BP test p-value =", 
                                      HETER_MODEL$
                                        test(heter_model(b = 64)$
                                               gen(300, computed = select(dat, x, e)))$p_value %>% 
                                        format(digits = 2))),
                      mod$
                        plot(heter_model(b = 0, e = rand_lognormal(sigma = 0.5))$
                               gen(300, computed = select(dat, x)), theme = theme_light()) + 
                        ggtitle(paste("Non-normality", 
                                      shapiro.test(heter_model(b = 0, e = rand_lognormal(sigma = 0.5))$
                                                     gen(300, computed = select(dat, x))$.resid)[[2]] %>% 
                                        format(digits = 2))))
