...
<!-- What truly impress us is the huge difference between the estimated power of visual test and the estimated power of F-test. According to the example lineups included in Figure \ref{fig:power-poly-uniform}, at around $log_e(E) = 1$, the  designed visual feature is rarely visible, making the actual data plot indistinguishable from residual plots simulated from the assumed model at this level of difficulty. In fact, zero out of 11 subjects detect the actual data plot positioned at panel 10. However, F-test has more than 60% chance of rejecting the null hypothesis in this case. From a communication perspective, given the fact that the visual difference is unperceivable, the argument that non-linearity present in the fitted model is less convincing to the public. -->

<!-- The margin is largest at around $log_e(E) = 2$. As shown in Figure \ref{fig:power-poly-uniform}, the visual pattern is still undetectable which is similar to $log_e(E) = 1$. The visual test power is close to zero. In contrast, F-test will reject the null hypothesis 90% of the time. -->

<!-- At around $log_e(E) = 3$, the margin gets smaller as the chance of identifying the actual data plot becomes larger. At this level of difficulty, the designed visual features are usually detectable but it may not stand out from the lineup as other null plots may happen to include outliers or visual patterns that are are considered to be more attractive by human, and thus recognized as the most different plot. Without knowing the designed visual features beforehand, it is actually hard to identify the actual data plot by pure image comparison. The corresponding example lineup for $log_e(E) = 3$ shown in Figure \ref{fig:fig:power-poly-uniform} has the actual data plot positioned at panel 20, where two of 11 subjects detect it. It can be observed that a "S-shape" is presented in plot 20, but the signal is not strong enough to attract all 11 subjects, resulting in a visual p-value sightly above $0.1$. At $log_e(E) = 4$, the designed visual features become much clear and attractive in panel 3, leading to a high percentage of rejection of the null hypothesis. Figure \ref{fig:fig:power-poly-uniform} gives example lineups of such cases. It is also interesting to observe that the power curve of visual test behaves similarly to those inappropriate conventional tests until the designed visual features really becomes visible. -->
...
p <- vi_survey %>%
  ...
  geom_smooth(data = mutate(poly_conv_sim, log_effect_size = log(effect_size)) %>%
              ...
              method.args = list(family = binomial),
              se = FALSE) +
  geom_point(aes(log_effect_size, prop_detect), alpha = 0.3)  +
