...
Visual inference and conventional hypothesis testing are two distinct statistical procedures. As displayed in Figure \ref{fig:p-value-comparison}, the visual test gives higher p-values than the conventional test in both polynomial and heteroskedasticity models in most of the time, suggesting that humans have lower confidence and much higher tolerance of the residual departures than the conventional test. 

In particular, given 95% confidence level, both tests will reject the null hypothesis in only around 58% of the cases. And in 41% of the cases, the visual test does not reject the null hypothesis but the conventional test does. In these 41% of the cases, though we know that the designed lineups contain various degrees of residual departures, analysts and the general public as the consumers of the output may not believe in the rejection of the null hypothesis, or at least may not consider the model violation as impactful, simply due to the fact that the departures are not clearly visible to them.

This makes it sounds like people are arrogantly ignoring the accuracy of conventional tests and trust too much to their visual perception, but it is not. In fact, the sensitivity of the conventional test could sometimes distract and discourage analysts from finding a good linear approximation to the data. The rejection of the null hypothesis because of acceptable and negligible residual departure is not practically useful and thus is usually viewed as a "false alarm" by analysts. This also partly explains why abundance of literature suggesting the use of data plot in regression diagnostics.

One noticeable difference compared to the result provided in @majumder_validation_2013 is we do not observe the pattern from Figure \ref{fig:p-value-comparison} that when the conventional test p-value is small, the visual test p-value will be very small, and when the conventional test p-value is large, the visual test p-value will be very large. This could because the number of subjects involved in a lineup evaluation is much larger in their experiments with 46 as the average number. As mentioned in section [Experimental setup], our lineups have at most 11 evaluations due to the time and budge constraint. Besides, evaluating a visual test with more than 15 people is rare given the size of research or business team.


```{r p-value-comparison, fig.height=4, fig.cap="..."}
vi_survey %>%
  filter(!null_lineup) %>%
...
vi_survey %>%
  ...
  geom_point(aes(log_effect_size, prop_detect), alpha = 0.3) + geom_point(data = ~filter(.x, unique_lineup_id %in% c("poly_502", "poly_78", "poly_6", "poly_447")), aes(log_effect_size, prop_detect), alpha = 1, col = "red", size = 2) +
  stat_smooth(geom = "line", 
              data = ~boot_y(.x, reject_dirichlet, times = 500), 
              aes(log_effect_size, reject_dirichlet, group = boot_id, col = "Visual p-value"), 
              method = "glm", 
...
VI_MODEL$plot_lineup(vi_lineup[["poly_502"]]$data, 
                     remove_grid_line = TRUE, 
                     theme = theme_light(base_size = 5),
                     remove_axis = TRUE) + 
  ggtitle(bquote(log[e](E) %~~% 1~","~"F-test p-value" == .(format(filter(vi_survey, unique_lineup_id == "poly_502")$conventional_p_value[1], digits = 3))~", Visual p-value " == .(format(filter(vi_survey, unique_lineup_id == "poly_502")$p_value[1], digits = 3)))) -> p1

VI_MODEL$plot_lineup(vi_lineup[["poly_78"]]$data, 
                     remove_grid_line = TRUE, 
                     theme = theme_light(base_size = 5),
                     remove_axis = TRUE) + 
  ggtitle(bquote(log[e](E) %~~% 2~","~"F-test p-value" == .(format(filter(vi_survey, unique_lineup_id == "poly_78")$conventional_p_value[1], digits = 3))~", Visual p-value " == .(format(filter(vi_survey, unique_lineup_id == "poly_78")$p_value[1], digits = 3)))) -> p2

VI_MODEL$plot_lineup(vi_lineup[["poly_6"]]$data, 
                     remove_grid_line = TRUE, 
                     theme = theme_light(base_size = 5),
                     remove_axis = TRUE) + 
  ggtitle(bquote(log[e](E) %~~% 3~","~"F-test p-value" == .(format(filter(vi_survey, unique_lineup_id == "poly_6")$conventional_p_value[1], digits = 3))~", Visual p-value " == .(format(filter(vi_survey, unique_lineup_id == "poly_6")$p_value[1], digits = 3)))) -> p3

VI_MODEL$plot_lineup(vi_lineup[["poly_447"]]$data, 
                     remove_grid_line = TRUE, 
                     theme = theme_light(base_size = 5),
                     remove_axis = TRUE) + 
  ggtitle(bquote(log[e](E) %~~% 4~","~"F-test p-value" == .(format(filter(vi_survey, unique_lineup_id == "poly_447")$conventional_p_value[1], digits = 3))~", Visual p-value " == .(format(filter(vi_survey, unique_lineup_id == "poly_447")$p_value[1], digits = 3)))) -> p4
...
vi_survey %>%
  ...
  mutate(reject_dirichlet = as.numeric(p_value <= 0.05)) %>%
  mutate(log_effect_size = log(effect_size)) %>%
  filter(abs(log(effect_size) - 1) < 0.5) %>%
  filter(n == 100)
...
vi_survey %>%
  ...
  mutate(reject_dirichlet = as.numeric(p_value <= 0.05)) %>%
  mutate(log_effect_size = log(effect_size)) %>%
  filter(abs(log(effect_size) - 4) < 0.5) %>%
  filter(prop_detect < 0.25)

vi_survey %>%
  filter(unique_lineup_id == "poly_199") %>%
  View()


VI_MODEL$plot_lineup(vi_lineup$poly_159$data)
...
vi_survey %>%
  ...
  ggplot() +
  geom_abline() +
  geom_point(aes(p_value_uniform, p_value)) +
  scale_x_log10() +
  scale_y_log10()
...
vi_survey %>%
  ...
  mutate(log_effect_size = log(effect_size)) %>%
  ggplot() +
  ggbeeswarm::geom_quasirandom(aes(factor(e_sigma), weighted_detect)) +
  facet_wrap(~n)
```
