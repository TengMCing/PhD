---
title: |
  A plot is worth a thousand tests: assessing residual diagnostics with visual inference
...
\noindent where $\boldsymbol{V}$ is the covariance matrix of the error term. 
...
To compute the effect size for each lineup we simulate ??
...
As expected, if the fitted value distribution is not uniform, there is a loss of power in the visual test. Structure is hardest to detect if fitted values are lognormal. Also, complex structure are generally harder to detect, but there are outliers. A surprising finding was that the direction of heteroskedasticity appears to affect the ability to visually detect it, with wedge to the right being less detectable.
