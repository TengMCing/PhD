...
Three experiments are conducted to investigate the difference between conventional hypothesis testing and visual inference in the application of linear regression diagnostics. Two types of departures, namely non-linearity and heteroskedasticity, are considered with the corresponding data generating process being designed for experiment I and II. The experiment III is designed for collecting human responses to null lineups such that the parameter $\alpha$ in Equation \ref{eq:pvalue-beta-binomial} can be estimated. Overall, we plan to collect `r vi_survey  %>% filter(!attention_check) %>% nrow()` evaluations on `r vi_survey  %>% filter(!attention_check) %>% pull(unique_lineup_id) %>% unique() %>% length()` unique lineups performed by `r vi_survey  %>% filter(!attention_check) %>% count(exp, set) %>% nrow()` subjects throughout three experiments. 
...
data.frame(j = c("2", "3", "6", "18", ""),
  ...
  knitr::kable(col.names = NULL,
               ...
               format = "latex",
               booktabs = TRUE, 
               caption  = 'Levels of the factors used in experiments I, II, III.', 
...
Figure \ref{fig:example-poly-lineup} demonstrates one of the lineups used in experiment I. This lineup is produced by the non-linearity model with $j = 6$ and uniform fitted values. The data plot location is $2^3 - 4$. All five subjects correctly identify the data plot from this lineup.

```{r different-n, fig.height = 2.67, fig.cap = "Number of observations $n$ have an impact on the strength of the signal. Three different values of $n$ are used in experiment I, II and III."}
...
```{r different-sigma, fig.height = 2, fig.cap = "For the non-lienearity model, the standard deviation of the error term $\\sigma$ affects the strength of the signal. Four different values of $\\sigma$ are used in the experiment I."}
...
```{r example-poly-lineup, fig.height = 7, fig.width = 7, fig.cap = "One of the lineups containing non-linearity patterns used in experiment I. Can you spot the most different plot? The data plot is positioned at $2^3 + 1$."}
...
```{r example-heter-lineup, fig.height = 7, fig.width = 7, fig.cap = "One of the lineups containing heteroskedasticity pattern used in experiment II. Can you spot the most different plot? The data plot is positioned at $3^3 - 3^2$"}
...
### Factors common to both experiments
...
Therefore, we plan to collect 6 more evaluations on the 279 lineups with uniform fitted values, result in $(36 \times 20 + (4 \times 4 \times 3 + 3 \times 5 \times 3) \times 3 \times 6) / (20-2) = `r (36 * 20 + (4 * 4 * 3 + 3 * 5 * 3) * 3 * 6) / 18`$ subjects recruited for experiment III.
...
map_df(levels(dat_poly_j$shape), function(x) {
  
  # Adjust the range of x
  result <- data.frame(log_effect_size = seq(min(dat_poly_j$log_effect_size),
                                             max(dat_poly_j$log_effect_size), 
                                             0.01))
  
  # Manually fit the model
  conv_mod <- glm(reject ~ log_effect_size, 
                  family = binomial(),
                  data = filter(dat_poly_j, 
                                shape == x, 
                                conv_or_not == "Conventional"))
  
  # Manually fit the model
  visual_mod <- glm(reject ~ log_effect_size, 
                  family = binomial(),
                  data = filter(dat_poly_j, 
                                shape == x, 
                                conv_or_not == "Visual"))
  ...
  # Get the prediction
  result <- result %>%
    mutate(Conventional = predict(conv_mod, 
                                  newdata = result, 
                                  type = "response"),
           Visual = predict(visual_mod, 
                            newdata = result, 
                            type = "response"),
           shape = x) %>%
    pivot_longer(Conventional:Visual, names_to = "conv_or_not")
  
  result
}) %>%
  ggplot() +
  geom_smooth(aes(log_effect_size, value, col = shape), se = FALSE, span = 0.03) +
  xlab(expression(log[e] (Effect_size))) +
  ylab("Power") +
...
We also investigate the impact of different heteroskedasticity patterns. In theory, the "Left-triangle" and the "Right-triangle" should have little difference from the point of view of a Breusch-Pagan test. According to Figure \ref{fig:heter-power-uniform-a}, this is indeed the case. 
Similarly, these two shapes should have the same visual test power, unless the orientation of the visual pattern is a non-negligible factor from human perspective. However, it can be observed from Figure \ref{fig:heter-power-uniform-a} that the power curve for the "Left-triangle" shape is constantly higher than the one for the "Right-triangle" shape, indicating a potential favour of orientation by human, which is worth to be explored in future studies.
...
map_df(levels(dat_heter_a$a), function(x) {
  
  # Adjust the range of x
  result <- data.frame(log_effect_size = seq(min(dat_heter_a$log_effect_size),
                                             max(dat_heter_a$log_effect_size), 
                                             0.01))
  
  # Manually fit the model
  conv_mod <- glm(reject ~ log_effect_size, 
                  family = binomial(),
                  data = filter(dat_heter_a, 
                                a == x, 
                                conv_or_not == "Conventional"))
  ...
  ggplot() +
  geom_smooth(aes(log_effect_size, value, col = a), se = FALSE, span = 0.03) +
  xlab(expression(log[e] (Effect_size))) +
  ylab("Power") +
...
## Effect of other fitted value distributions

### Fitted value distribution 

The prediction in a regression model is $E(Y|X)$, that is, it is conditional on observed values of the predictors. The distribution of $X$, or consequently $\hat{Y}$, may however affect the ability to read any patterns in the residuals. <!-- Although the distribution of the regressor is classically not restricted by the assumptions of linear regression, it will have an impact on the fitted value distribution and the visual effect of the residual plot.--> The four distributions of fitted values, used in this experiment, were designed to examine this.
...
For non-linearity patterns, we do not observe significant difference in power of visual test between distributions, except for the discrete uniform distribution. It is possibility due to the fact that humans have difficulties in recognising visual patterns from discreteness, which makes the shape disconnected and incomplete (ref?). In terms of conventional tests, the discrete uniform distribution is instead the one with the greatest power, followed by the lognormal distribution, the uniform distribution and the normal distribution.
