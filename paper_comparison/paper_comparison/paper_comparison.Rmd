...
Residuals might be plotted, as a histogram or quantile-quantile plot to examine the distribution. Using the classical linear regression model as an example, if the distribution is symmetric and unimodal, it is well-behaved. But if the distribution is skewed, bimodal, multimodal, or even contains outliers, there is cause for concern. Conducting a goodness of fit test is an alternative method for inspecting the distribution, such as the Shapiro-Wilk Normality test [@shapiro1965analysis].

To scrutinize the relationship between residuals, predicted values and each of the explanatory variables, it is recommend to plot them on a scatter plot. In general, the model is well-behaved if there is any visual discoverable pattern. However, it is a very difficult task for a human judge, though to make a decision that there's nothing there. It is especially common, particularly among new data analysts to report patterns when an experienced data analyst might quickly conclude that there are none. Generally, one looks for departures from nothingness like non-linear dependency or heteroskedasticity. It is also possible to conduct hypothesis tests for non-linear dependence [@ramsey_tests_1969], and also use a Breusch-Pagan test [@breusch_simple_1979] for heteroskedasticity.
...
dat <- mod$gen(300)
patchwork::wrap_plots(mod$plot(dat, theme = theme_light()) + ggtitle("Good residuals"),
                      mod$plot(mod$set_prm("include_z", TRUE)$gen(300, computed = select(dat, x, e)), theme = theme_light()) + ggtitle("Non-linearity"),
                      mod$plot(heter_model(b = 64)$gen(300, computed = select(dat, x, e)), theme = theme_light()) + ggtitle("Heteroskedasticity"),
                      mod$plot(heter_model(b = 0, e = rand_lognormal(sigma = 0.5))$gen(300, computed = select(dat, x)), theme = theme_light()) + ggtitle("Non-normality"))
```

Graphical summaries in which residuals are plotted against fitted values or other functions of the predictor variables that are approximately orthogonal to residuals are referred to as standard residual plots in @cook1982residuals. As shown in Figure [ref here], the top-left panel is a good residual plot with residuals evenly distributed at both sides of the horizontal zero line showing no noticeable patterns.

There are many types of departures from a good residual plot. Three of them will be considered in this paper, which are also the most commonly checked departures for a residual plot. The first one is non-linearity. Any unnatural and non-linear functional form of residuals on fitted values presented in the residual plot could be considered as an indicative of non-linearity. At the top-right of Figure [ref here], there is an example residual plot of this type of departure. The second type of departure that will be considered is heterskedasticity. The usual pattern is the inconsistent distribution of the residuals for different x values. Visually, it sometimes results in the so-called "butterfly" shape as shown in the bottom-left panel of Figure [ref here], or the "triangle" shape where the smallest variance occurs at the edges of the x-axis. The last type of departure we will consider is non-normality. This departure is usually harder to be detected from the residuals plot compared to non-linearity and heterskedasticity. In most cases, a quantile-quantile plot can do a better job. Depending on the actual distribution of the residuals, different visual patterns will be discovered. Readers of the residuals plot will notice the data points do not distribute noramlly on the y-axis. For example, if the distribution is skewed, one will see the "tail" of the distribution as some outliers presented on only one side of the zero line as shown in the bottom-right of Figure [ref here].
