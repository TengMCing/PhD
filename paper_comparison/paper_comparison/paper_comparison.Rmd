...
and visual tests reject `r p_value_cmp_dat %>% filter(type == "non-linearity") %>% count(reject) %>% summarise(pct=round(n[1]/(n[1]+n[2])*100, 0))`% of the time. Of the tests rejected by the conventional test, `r p_value_cmp_dat %>% filter(type == "non-linearity") %>% filter(conv_reject == "Reject") %>% count(reject) %>% summarise(pct=round(n[1]/(n[1]+n[2])*100, 0))`%, so approximately half as many as the conventional test. Surprisingly, the visual test rejects `r p_value_cmp_dat %>% filter(type == "non-linearity") %>% filter(conv_reject == "Not") %>% count(reject) %>% summarise(pct=round(n[1]/(n[1]+n[2])*100, 0))`% of lineups where the conventional test does not reject.

are rejected by both the conventional and visual tests, while 42% are accepted by both tests. Thus, 69% of the time two tests will agree with each other. There are 26% of lineups rejected by conventional tests only.
