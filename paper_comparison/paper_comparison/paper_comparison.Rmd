...
Figure \ref{fig:different-x-dist-poly-power} illustrates the change of power of visual tests for different fitted distributions used in the non-linearity model and the heteroskedasticity model. Note that we have collected more evaluations on lineup with uniform fitted value distribution, and the number of evaluations on a lineup will have an impact on the power of the visual test. To have a fair comparison, the five evaluations used in the power estimation are randomly sampled from the total eleven evaluations for each lineup with uniform fitted value distribution. The sampling process has been repeated 500 times to give an indication of the variation of the power.


We do not observe significant difference between the power curve of the uniform distribution and the power curve of the discrete uniform distribution, but the power curves of normal and lognormal distributions are slightly lower. Same can be observed for heteroskedasticity patterns, where the gap between the power curve of the discrete uniform and the power curve of the normal distribution is even larger. This indicates that residual departures displayed with evenly distributed fitted values are visually more attractive. Besides, the power of detecting non-linearity or heteroskedasticity patterns from a lineup with lognormal fitted value distribution is the lowest among four distributions. This suggests the skewness of the fitted value distribution plays a role in the degree of presence of visual features, and a symmetric fitted value distribution can better reveal the underlying shape.  
