...
Pr(\text{reject}~H_0|H_1,E) = \Lambda(log(\frac{0.05}{0.95}) + \beta_1 E),
\end{equation}
```
\noindent where $\Lambda(.)$ is the standard logistic function given as $\Lambda(z) = exp(z)/(1+exp(z))$. The effect size is the only regressor and the intercept is fixed to $log(0.5/0.95)$ so that $\hat{Pr}(\text{reject}~H_0|H_1,E = 0) = 0.05$.

Effect size $E$ is derived from the Kullback-Leibler divergence (see Appendix for the details) formulated as
