...
vi_survey_processed %>%
  ...
  geom_point(aes(log_effect_size, prop_detect), alpha = 0.3) + geom_point(data = ~filter(.x, unique_lineup_id %in% c("poly_42", "poly_562", "poly_217", "poly_120")), aes(log_effect_size, prop_detect), alpha = 1, col = "red", size = 2) +
...
VI_MODEL$plot_lineup(vi_lineup[["poly_42"]]$data, 
                     remove_grid_line = TRUE, 
                     theme = theme_light(base_size = 5),
                     remove_axis = TRUE) + 
  ggtitle(bquote(log[e](E) %~~% 2~","~"F-test p-value" == .(format(filter(vi_survey_processed, unique_lineup_id == "poly_42")$conventional_p_value[1], digits = 3))~", Visual p-value " == .(format(filter(vi_survey_processed, unique_lineup_id == "poly_42")$p_value_select_all_plots[1], digits = 3)))) -> p1

VI_MODEL$plot_lineup(vi_lineup[["poly_562"]]$data, 
                     remove_grid_line = TRUE, 
                     theme = theme_light(base_size = 5),
                     remove_axis = TRUE) + 
  ggtitle(bquote(log[e](E) %~~% 3~","~"F-test p-value" == .(format(filter(vi_survey_processed, unique_lineup_id == "poly_562")$conventional_p_value[1], digits = 3))~", Visual p-value " == .(format(filter(vi_survey_processed, unique_lineup_id == "poly_562")$p_value_select_all_plots[1], digits = 3)))) -> p2

VI_MODEL$plot_lineup(vi_lineup[["poly_217"]]$data, 
                     remove_grid_line = TRUE, 
                     theme = theme_light(base_size = 5),
                     remove_axis = TRUE) + 
  ggtitle(bquote(log[e](E) %~~% 4~","~"F-test p-value" == .(format(filter(vi_survey_processed, unique_lineup_id == "poly_217")$conventional_p_value[1], digits = 3))~", Visual p-value " == .(format(filter(vi_survey_processed, unique_lineup_id == "poly_217")$p_value_select_all_plots[1], digits = 3)))) -> p3

VI_MODEL$plot_lineup(vi_lineup[["poly_120"]]$data, 
                     remove_grid_line = TRUE, 
                     theme = theme_light(base_size = 5),
                     remove_axis = TRUE) + 
  ggtitle(bquote(log[e](E) %~~% 2~","~"F-test p-value" == .(format(filter(vi_survey_processed, unique_lineup_id == "poly_120")$conventional_p_value[1], digits = 3))~", Visual p-value " == .(format(filter(vi_survey_processed, unique_lineup_id == "poly_120")$p_value_select_all_plots[1], digits = 3)))) -> p4
...
vi_survey_processed %>%
  ...
  mutate(reject_dirichlet = as.numeric(p_value_select_all_plots <= 0.05)) %>%
  mutate(log_effect_size = log(effect_size)) %>%
filter(abs(log(effect_size) - 3) < 0.2)
