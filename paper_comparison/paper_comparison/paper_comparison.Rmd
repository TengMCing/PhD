...
vi_survey %>%
  ...
  summarise(across(everything(), first)) %>%
  mutate(reject = as.numeric(p_value < 0.05)) %>%
  mutate(reject_only_5  = as.numeric(p_value_only_5 < 0.05)) %>%
  mutate(log_effect_size = log(effect_size)) %>%
  ggplot() +
  geom_smooth(aes(log_effect_size, reject), 
              method = "glm", 
              method.args = list(family = binomial()),
              se = FALSE) +
  geom_smooth(aes(log_effect_size, reject_only_5), 
              method = "glm", 
              method.args = list(family = binomial()),
              col = "red",
              se = FALSE) +
...
```{r different-x-dist-poly-power, fig.cap = "Comparison of power on lineups with different fitted value distributions, for conventional tests and visual tests. Each power curve of the uniform distribution is surrounded by a grey line to indicate it is the baseline in its group. For both types of tests power does change. The discrete fitted value distribution substantially lowers the power of visual tests for non-linearity patterns in residual plots. For heteroskedasticity patterns, normal and lognormal distributions produce lower power for both types of tests."}
...
dat_x_dist %>%
  ...
  # Bold the uniform smooth line
  # geom_smooth(data = ~filter(.x, x_dist == "Uniform"),
  #             aes(log_effect_size, reject), 
  #             col = "grey", 
  #             se = FALSE, 
  #             span = 0.03,
  #             size = 3) +
  ...
  geom_smooth(data = ~filter(.x, x_dist != "Uniform"),
              aes(log_effect_size, reject, col = x_dist), 
              span = 0.03,
              se = FALSE, 
              size = 1) +
  
  geom_smooth(data = vi_survey %>%
                filter(!null_lineup) %>%
                filter(!attention_check) %>%
                filter(x_dist == "uniform") %>%
                group_by(unique_lineup_id) %>%
                ...
                mutate(type = ifelse(type == "polynomial", "non-linearity", type)) %>%
                mutate(conv_or_not = "Visual"),
              aes(log_effect_size, reject_only_5, col = "Uniform"),
              method = "glm",
              method.args = list(family = binomial()),
              se = FALSE
              ) +
