...
es <- map_dbl(target_sigma, ~log(poly_model(shape = 2, x = rand_uniform(), sigma = .x)$effect_size(n = 100)))
```
...
p <- vi_survey %>%
  ...
  geom_segment(data = data.frame(xi = es), aes(x = xi, xend = xi, y = 0, yend = 1)) +
  theme_light(base_size = 5) +
  scale_color_brewer(palette = "Dark2", direction = -1) +
  xlab("Natural logarithm of effect size") +
  ylab("Power")
...
patchwork::wrap_plots(append(plist, list(p), after = 0), 
                      design = map(1:11, ~patchwork::area(11, .x, 11, .x)) %>%
                        reduce(c) %>%
                        c(patchwork::area(1, 1, 10, 11), .))
```
...
```{r fig.height = 3, fig.width = 12, fig.cap="..."}
patchwork::wrap_plots(append(plist, list(p), after = 0), 
