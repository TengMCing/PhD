...
Two experiments were conducted to evaluate the effectiveness of visual
test relative to the equivalent conventional test in regression
diagnostics. The first experiment has ideal scenario for conventional
testing, where we would not expect the visual test to do better than the
conventional test. The second experiment is a scenario where the
conventional test is an approximate test, which visual test may have a
chance to match the performance of the conventional test.

As suggested by many previous studies {[}many studies refs here{]}, it
is clear some care is needed to set up the human subjects experiments.
It is best for an subject to evaluate a block of 20 lineups with varying
difficulty, with a reasonable number of ``easy'' lineups. The
explanations about each experiment includes an explantion of how the
lineups were sampled and provided to the subjects.

Participants for all the experiments were recruited through Prolific's
online web service {[}ref here{]}. A summary of the data obtained for
all three eperiments are shown in {[}table ref here{]}. Participants
were asked to select the plots which are mostly different from others,
provide a reason for their selections, and express how different they
think the selected plots are from others. Gender, age, education, and
previous experience in visual experiment are also collected. For each of
the experiments, \(12\) of the lineups were used as a attention check
which everyone will randomly get two of them and should get at least one
correct, so that a measure of the quality of the subjects effort could
be made. Note that no participant was shown the same lineup twice.

\hypertarget{non-linear-covariates}{%
\subsection{Non-linear covariates}\label{non-linear-covariates}}

The experiment is designed to study the ability of human subjects to
detect the effect of a higher order term \(Z\) in a two variable
statistical model formulated as:
...
\(\boldsymbol{\varepsilon}\), \(\boldsymbol{X_{raw}}\),
\(\boldsymbol{Z_{raw}}\) are \(n\times1\) matrices, \(He_{j}(.)\) is the
\(j\)th-order probabilist's Hermite polynomials,
\(j \in \{2, 3, 6, 18\}\),
\(\varepsilon \sim N(\boldsymbol{0}, \sigma^2\boldsymbol{I}_n)\),
\(\sigma \in \{0.5, 1, 2, 4\}\), and \(g(\boldsymbol{X}, k)\) is a
scaling function to enforce the support of the variable to be
\(\{-k, k\}\) defined as
...
The null regression model used to fit the data genearted by the above
...
out the high order term. Data were simulated using a range of values of
\(j\) as shown in {[}table ref here{]}, three different smaple sizes (n
= 50, 100, 300) and four different standard deviations of the error
(\(\sigma\) = 0.5, 1, 2, 4). The range of \(j\) was chosen so that
different shapes of non-linearity are included in the residual plot.
...
calculated for the conventional test. The values for \(X_{raw}\) were
randomly generated independently from one of the following four
distributions: (1) \(U(a = -1, b = 1)\), (2)
\(N(\mu = 0, \sigma = 0.3)\), (3)
\(lognormal(\mu = 0, \sigma = 0.6)/3 - 1\) and (4)
\(u\{a = 1, b = 5\}\).
