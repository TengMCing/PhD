...
(use uniform effect size? normal effect size? -\textgreater{} other
dist?)
...
variables leave out by the regression equation and
\(\boldsymbol{\beta}_z\) be the corresponding coefficients. A
conditional expectation is taken because \(E_{sample}\) depends on the
regressors \(\boldsymbol{X}\) and we want the effect size \(E\) to be
the same for the same data generating process. It is difficult to solve
the effect size \(E\) analytically, so we estimate it with the average
of \(E_{sample}\) over a very large sample size.
