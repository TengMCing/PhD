...
express how different they think the selected plots are from others. If
there was no noticeable difference between plots in a lineup, subject
could choose to select zero lineups without providing the reason.
...
\(N(0.0.3^2)\) as the distribution of \(X_{raw}\). The actual data plot
location is four. All five subjects correctly identified the actual data
plot for this lineup.
...
Subjects recruited from Prolific received a fixed payment for submitting
quality responses. However, some subjects will try to maximize their
earnings for minimum effort. If a subject objectively demonstrated clear
low-effort throughout the experiment, i.e., failed both attention
checks, we rejected the submission and the subject would not be paid.
