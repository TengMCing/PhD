...
\begin{verbatim}
##  [1]  TRUE FALSE  TRUE  TRUE FALSE FALSE  TRUE  TRUE  TRUE  TRUE  TRUE
\end{verbatim}
...
Other than the parameters summarised in Table
\ref{tab:model-parameter-table}, three additional parameters \(n\),
\(\sigma\) and \(b\) were used to control the strength of the signal in
experiment I and II so that different difficulty levels of lineups were
generated, and therefore, the estimated power curve would be smooth and
continuous. Three different sample sizes were selected (n = 50, 100,
300). In general, with fewer data points shown in a residual plot, the
chance of detecting the visual pattern will decrease. This can be
observed from Figure

\begin{figure}
\includegraphics[width=1\linewidth]{paper_comparison_files/figure-latex/different-n-1} \caption{Example residual plots of the linear regression model used in experiment I. Three different values of $n$ are included in the experiment to control the strength of the signal.}\label{fig:different-n}
\end{figure}
