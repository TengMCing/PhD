...
Effect size \(E\) is formulated as the conditional expectation of the
sample-based effect size \(E_{sample}\) derived from the
Kullback-Leibler divergence (see Appendix for the details) given by
...
\noindent where \(M\) is a given model that generates the data,
\(diag(.)\) is the diagonal matrix constructed from the diagonal
...
A conditional expectation is taken because \(E\) is designed to be
invariant to different realizations of \(\boldsymbol{X}\). However,
solving this conditional expectation analytically is very difficult as
\(\boldsymbol{X}\) can follow arbitrary distributions. Therefore, we
make an assumption that \(\boldsymbol{X}\) follows a particular
multivariate normal distribution

We make an assumption that all the realizations of \(\boldsymbol{X}\)
are generated from a multivariate normal distribution
\(N(\boldsymbol{0}, 0.3^2\boldsymbol{I})\) and then rescaled by the
min-max normalization. The estimate of \(E\) is obtained by taking the
average of the \(E_{sample}\) calculated for samples drawn from the
given model \(M\) repeatedly.
...
distribution of the regressor, type of the simulation model and the
...
To get an accurate estimate of the power of conventional tests, we
simulate 200000 samples evenly from the models used in the human subject
experiments, and record the \(p\)-values produced by every sample.
...
a line up, where it is embedded in a field of null plots. This enables a
...
fitted values ar discrete. Also, a surprising find was that the
direction of heteroskedasticity appears to affect the ability to
visually detect it, with wedge to the left being less detectable.
