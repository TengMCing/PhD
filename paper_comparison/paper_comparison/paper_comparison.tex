...
\(log_e(E)\), with a 5\% significance level. Lineups with
\(X_{raw} \sim U(-1,1)\) are focused in this section because visual
patterns are more likely to be revealed under the uniform distribution.
Besides, we have collected more responses for this distribution in
experiment III to produce more reliable and stable power curves. Power
analysis of other distributions can be found in Section {[}?{]}. At the
bottom of the figure \ref{fig:polypower}, there are a sequence of
example residual plots with increasing levels of \(log_e(E)\). Readers
...
significantly from visual tests in these two scenarios. Visual test have
much higher tolerance of the residual departures than the conventional
test. Since fail to reject \(H_0\) in a visual test usually means that
there are no obvious visual discoveries found in the residual plot,
...
This may limit the popularity of conventional tests in residual
...
reject \(H_0\) more often than visual tests. In this section, we explore
how often they agree with each other by investigate the rejection rates
based on \(p\)-values.

Depending on the natural logarithm of the effect, lineups are classified
into three categories: easy (\textless{} 1.5), moderate (1.5 - 3.5) and
difficult (\textgreater{} 3.5). Figure \ref{fig:p-value-comparison}
...
There are 16\% of lineups rejected only by the conventional test.

Interestingly, in spite of the greater power of the conventional test,
10\% of the lineups are rejected only by the visual test. This means
among those accepted by the conventional test, 51\% are rejected by the
visual test. Recall that the RESET test used in our analysis includes
powers of fitted values up to four. However, the ``M'' and ``Triple-U''
shapes constructed from the Hermite polynomials contain powers of
regressor up to six and 18 respectively. In fact, by including the
powers of fitted values up to six, all cases will be rejected by the
RESET test. And from the example plot displayed in Figure
\ref{fig:easy-poly-example}, we can observe panel three is distinctly
different from others exhibiting a ``M'' shape. It clearly suggests
users of the RESET test and any other residual-based conventional tests
that require the specification of variables of interest to check the
residual plot before conducting the test, such that the correct choice
of variables can be made.

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{paper_comparison_files/figure-latex/easy-poly-example-1} 

}

\caption{An example easy lineup produced by the non-linearity model not rejected by the conventional test but rejected by the visual test. Panel three exhibits strong non-linear pattern.}\label{fig:easy-poly-example}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{paper_comparison_files/figure-latex/easy-heter-example1-1} 

}

\caption{An example easy lineup produced by the heteroskedasticity not rejected by the conventional test but rejected by the visual test. Panel 17 exhibits strong heteroskedasticity pattern.}\label{fig:easy-heter-example1}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{paper_comparison_files/figure-latex/easy-heter-example2-1} 

}

\caption{An example easy lineup produced by the heteroskedasticity not rejected by the conventional test but rejected by the visual test. Panel 18 contains a distinct outlier.}\label{fig:easy-heter-example2}
\end{figure}
...
accepted by both, result in 69\% agreement rate. Lineups rejected by
only conventional test are 30\%. 1\% of lineups are accepted by the
conventional test but rejected by the visual test.

Among those 1\% lineups, which are two lineups, one shows detectable
heteroskedasticity pattern as displayed in Figure
\ref{fig:easy-heter-example1}. Another contain data plot with an
noticeable outlier, but the ``Right-triangle'' shape is not obvious as
shown in Figure \ref{fig:easy-heter-example2}. The \(p\)-values of
...
moderate lineup produced by the non-linearity model, only 8\% and 28\%
are rejected and accepted by both respectively. We can observe a decline
in agreement rate to 36\%. A huge amount of lineups are accepted by the
visual test. 63\% of lineups rejected only by the conventional test.
There are 1\% of lineups which rejected only by conventional tests
suffer the same RESET test issue as mentioned earlier.

For those produced by the heteroskedasticy model, the corresponding
numbers are 23\%, 31\%, 54\%, 45\% and 1\%. Only one lineup rejected by
the visual test but accepted by the conventional test. It shows zero
heteroskedasticity patterns, but contains a few outliers at the bottom
of the data plot as illustrated in panel 14 of Figure
\ref{fig:moderate-heter-example1}. Two out of five subjects correctly
identify this lineup yielding a \(p\)-value equal to \(0.044\).

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{paper_comparison_files/figure-latex/moderate-heter-example1-1} 

}

\caption{An example easy lineup produced by the heteroskedasticity not rejected by the conventional test but rejected by the visual test. Panel 14 contains a few outliers at the bottom of the panel.}\label{fig:moderate-heter-example1}
\end{figure}
...
to difficult lineups produced by the non-linearity model, none of them
are rejected by the visual test. There are 73\% of lineups accepted by
both tests, and 26\% rejected by the conventional test. The agreement
rate rise up to 73\%.

For lineups produced by the heteroskedasticity model, 3\% are rejected
by both, 71\% are accepted by both, 24\% are rejected only by the
conventional test and 2\% are rejected only by the visual test, which
are two lineups.
...
designed patterns are ``Left-triangle'' and ``Butterfly'', both of them
are invisible due to the small effect. The residual plots are dominated
by the skewed distribution. Further, no distinguishable visual patterns,
such as outliers, are detectable from the data plot of these two
lineups, which are panel 5 and panel 15. The data plot in these two
...
reasons for selecting the plot provided by the subjects are not
consistent. Therefore, with the current data, we can not explain why
...
{\centering \includegraphics[width=1\linewidth]{paper_comparison_files/figure-latex/difficult-heter-example1-1} 

}

\caption{An example easy lineup produced by the heteroskedasticity not rejected by the conventional test but rejected by the visual test. Panel 14 contains a few outliers at the bottom of the panel.}\label{fig:difficult-heter-example1}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{paper_comparison_files/figure-latex/difficult-heter-example2-1} 

}

\caption{An example easy lineup produced by the heteroskedasticity not rejected by the conventional test but rejected by the visual test. Panel 14 contains a few outliers at the bottom of the panel.}\label{fig:difficult-heter-example2}
\end{figure}
