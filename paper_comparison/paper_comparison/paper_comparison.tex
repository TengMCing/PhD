...
Shape of the non-linearity pattern is a primary factor contributes to
the non-linearity model. With different non-linearity patterns, it is
expected that the visual test will respond variously.

According to Figure \ref{fig:poly-power-uniform-j}, conventional tests
have almost identical power in testing the ``U'', the ``S'' and the
``M'' shape, except for testing the ``Triple-U'' shape, which have
significantly lower power. To understand why this is, one needs to
return to the way the RESET test is applied. It requires a parameter
...
for the ``M'' shape. Interestingly, the ``M'' shape is as easy to be
detected as the ``U'' shape, which implies the readability of the shape
do not strictly follow the degree of the polynomials.
...
We also investigate the impact of different heteroskedasticity patterns.
In theory, the ``Left-triangle'' and the ``Right-triangle'' should have
little difference from the point of view of a Breusch-Pagan test.
According to Figure \ref{fig:heter-power-uniform-a}, this is indeed the
case. Similarly, these two shapes should have the same visual test
power, unless the orientation of the visual pattern is a non-negligible
factor from human perspective. However, it can be observed from Figure
\ref{fig:heter-power-uniform-a} that the power curve for the
