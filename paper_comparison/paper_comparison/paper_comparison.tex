...
\textbackslash ref\{fig:commondepartures\}, the top-left panel is a good
...
types of departures from a good residual plot. We will only discuss
three most commonly checked departures in this paper. Namely,
non-linearity, heterskedasticity and non-normality.
...
Figure \textbackslash ref\{fig:commondepartures\}, or the
``left-triangle'' and ``right-triganle'' shape where the smallest
variance occurs at the edges of the x-axis.
...
in the bottom-right of Figure
\textbackslash ref\{fig:commondepartures\}.
...
examined by the corresponding F-test, Breusch-Pagan test and
...
hypothesis for departures they do not intend to examine. As discussed in
\citet{cook1982residuals}, most residual-based tests for a particular
type of departure from model assumptions are sensitive to other types of
departures. It is likely the null hypothesis is correctly rejected but
for the wrong reason, which is known as the ``Type III error''.
Additionally, outliers will often incorrectly trigger the rejection of
the null hypothesis despite the residuals are well-behaved
...
\caption{\label{tab:example-residual-plot-table}Example residual plots examined by the corresponding conventional tests. P-values smaller than 0.05 are colored in red.}
\centering
\begin{tabular}[t]{lllll}
\toprule
Residual plots & Departures & F-test  & Breusch-Pagan test & Shapiro–Wilk test\\
\midrule
B & Non-linearity & \textcolor{red}{7.3e-117} & \textcolor{red}{1.8e-14} & \textcolor{red}{3.9e-02}\\
C & Heteroskedasticity & \textcolor{black}{ 3.8e-01} & \textcolor{red}{6.1e-16} & \textcolor{red}{1.1e-09}\\
D & Non-normality & \textcolor{black}{ 6.7e-01} & \textcolor{black}{7.4e-01} & \textcolor{red}{2.9e-18}\\
\bottomrule
\end{tabular}
\end{table}
...
out the most different plot from the lineup. Figure
\ref{fig:first-example-lineup} is an example of a lineup protocol. If
the actual data plot at position 11 is identifiable, then it is evidence
for the rejection of the null hypothesis that the regression model is
correctly specified. In fact, the example lineup is generated from a
misspecified regression model with non-linearity issue. So one may
notice the cubic curve in the actual residual plot.

Simulation of the null data used in the 19 null plots are

\begin{figure}
\includegraphics[width=1\linewidth]{paper_comparison_files/figure-latex/first-example-lineup-1} \caption{Example lineup consists of 20 randomly placed residual plots, where 19 are null plots with data consistent with the null hypothesis, and one is the actual residual plot. Can you find the most different plot?}\label{fig:first-example-lineup}
\end{figure}
