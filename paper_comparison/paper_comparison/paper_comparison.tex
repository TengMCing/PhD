...
rejected by conventional test are 20\%.

Surprisingly, the visual test rejects 3\% (i.e.~1 out of 32) of lineups
where the conventional test does not reject. This lineup shows
relatively strong heteroskedasticity pattern as displayed in Figure
\ref{fig:heter-example}. The visual test rejects it with a
\(p\text{-value} = 0.026\), but the Breusch-Pagan test produces a
\(p\)-value slightly above 5\%.
