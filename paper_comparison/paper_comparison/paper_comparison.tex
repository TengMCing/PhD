...
\(\boldsymbol{\beta}_z\) be the corresponding coefficients. An
expectation is taken because \(E_{sample}\) depends on the regressors
\(X\) and we want the effect size \(E\) to be the same for the same data
generating process. It is difficult to solve the effect size \(E\)
analytically, so in the analysis, we estimate it with the average of
\(E_{sample}\) over a very large sample size.
