...
\caption{\label{tab:example-residual-plot-table}Example residual plots examined by the corresponding conventional tests. P-values smaller than 0.05 are showed in bold.}
\centering
\begin{tabular}[t]{llrrr}
\toprule
Residual plots & Departures & Model structural & Breusch-Pagan & Shapiro–Wilk\\
\midrule
...
test. Meanwhile, as mentioned in the \ref{introduction}
\protect\hyperlink{introduction}{introduction} section

For the purpose of investigating the differences between conventional
hypothesis testing and graphical diagnostics in the application of
linear regression diagnostics, three experiments were conducted.
...
the performance of the conventional test.
...
This should be a section that summarizes the background, gives the
motivation to the experiments, and most importantly reflects on the
title.
