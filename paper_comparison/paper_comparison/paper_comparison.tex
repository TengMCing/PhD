...
matrices. The model is a logistic regression formulated as
...
as \(\Lambda(z) = exp(z)/(1+exp(z))\). The effect size is the only
regressor and the intercept is fixed to \(log(0.5/0.95)\) so that
\(\hat{Pr}(\text{reject}~H_0|H_1,E = 0) = 0.05\).

Effect size \(E\) is derived from the Kullback-Leibler divergence (see
...
periods. The two types of departures, non-linearity and
heteroskedasticity, are primarily collected during data collection
periods I and II. The data collection period III was designed primarily
to measure human responses to null lineups so that the parameter
\(\alpha\) in Equation \ref{eq:pvalue-beta-binomial} can be estimated.
Additional lineups for both non-linearity and heteroskedasticity, using
uniform \(X\) distribution, were included so that the participants were
evaluating some lineups with signal also. It would be too frustrating
for participants to only be assigned lineups with all null plots.
Overall, we collected 7974 evaluations on 1152 unique lineups performed
by 443 subjects throughout three data collection periods.
...
\ref{effect-of-fitted-value-distributions}.
...
aggressively from 7\% to 63\% as \(log_e(E)\) increases from 0 to 2.5,
...
depatures can be spotted from the example residual plots.
...
\(log_e(E)\) at around 2.5, where the power curve of the visual test
...
\ref{fig:poly-power-uniform-j}, conventional tests have identical power
in testing the ``U'', the ``S'' and the ``M'' shape. But it has
significant lower power in testing the ``Triple-U'' shape. To understand
why this is, one needs to return to the way the RESET test is applied.
It requires a parameter indicating degree of fitted values to test for,
and the recommendation is to generically use four
...
``Triple-U'' shape will have little difference to other shapes. The
...
the data generating process in order to set the parameters for the test.
...
shape, which is as easy to be detected as the ``U'' shape. This implies
the readability of the shape do not strictly follow the degree of the
polynomials.
...
visual tests should have the smae power in detecting these two shapes if
...
chance of detecting the ``Butterfly'' shape by visual tests are slightly
higher than other two shapes.
...
with the greatest power, followed by lognormal, uniform and the normal
distribution.
...
periods using two simulation models, one for for non-linearity and and
the other for heteroskedasticity. Methods for obtaining the \(p\)-value
of visual tests proposed by \citet{vanderplas2021statistical} are
extended to allow for multiple selections in a lineup protocol.
...
According to the data collected from the human subject experiment,
residual plots rejected by visual tests are rarely not rejected by
conventional tests, but departures of residuals that are of little
practical sense to humans are sometimes alarmed by conventional tests,
supporting the common guidance given by experts to utilize graphical
representations of data in model diagnostics. We have also found that
residual-based conventional tests relying on a user-parameterized test
formula, such as the RESET test, will suffer from a significant decrease
in power for detecting certain residual departures intended to be tested
but not covered by the test formula. This is difficult to be avoided
without prior knowledge of their functional forms, which is usually
obtained by plotting the residuals. On the other hand, visual tests do
not rely on such information and a collection of departures can be
tested simultaneously. Interestingly, subjects recruited for our
experiment behaves a potential favour of the orientation of the residual
departure patterns. They perform slightly better in identifying triangle
residual patterns pointing to the left than to the right. Besides, it is
found that fitted value distributions have a great impact on the power
of both conventional and visual tests. Generally, fitted values followed
an evenly distributed and symmetric distribution such as the uniform
distribution is preferred. Depending on the type of the residual
departures, discreteness may cause trouble to humans to identify the
residual patterns, while conventional tests are not affected. Skewness
on the other hand significantly decrease the power of both conventional
and visual tests when testing heteroskedasticity. Although it is
commonly assumed by analysts, a normal fitted value distribution may not
be the optimal option when comes to regression diagnostics.
